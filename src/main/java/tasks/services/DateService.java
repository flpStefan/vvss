...
public class DateService {
    ...
    public Date getDateValueFromLocalDate(LocalDate localDate) {
        if (localDate == null) {
            System.out.println("Warning: LocalDate is null, returning null.");
            return new Date(); // or return new Date(); to default to current date
        }
    ...
    public Date getDateMergedWithTime(String time, Date noTimeDate) {//to retrieve Date object from both DatePicker and time field
        ...
        if (hour < 0 || hour >= HOURS_IN_A_DAY || minute <0 || minute >= MINUTES_IN_HOUR) throw new IllegalArgumentException("time unit exceeds bounds");
